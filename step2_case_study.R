...
#  26    24            123           26       77
# without prior knowledge    
CSlncR_network_src_  <- CSlncR_network_
...
df_GW16 <- unique(CSlncR_merge_GW(CSlncR_network_GW16))
df_GW19.5 <- unique(CSlncR_merge_GW(CSlncR_network_GW19.5))
df_GW20.5 <- unique(CSlncR_merge_GW(CSlncR_network_GW20.5))
df_GW21 <- unique(CSlncR_merge_GW(CSlncR_network_GW21))
df_GW23.5 <- unique(CSlncR_merge_GW(CSlncR_network_GW23.5))
CSlncR_network_GW16to23.5 <- list(df_GW16, df_GW19.5, df_GW20.5, df_GW21, df_GW23.5)
CSlncR_network <- CSlncR_network_GW16to23.5
...
df_GW16_ <- unique(CSlncR_merge_GW(CSlncR_network_GW16_))
df_GW19.5_ <- unique(CSlncR_merge_GW(CSlncR_network_GW19.5_))
df_GW20.5_ <- unique(CSlncR_merge_GW(CSlncR_network_GW20.5_))
df_GW21_ <- unique(CSlncR_merge_GW(CSlncR_network_GW21_))
df_GW23.5_ <- unique(CSlncR_merge_GW(CSlncR_network_GW23.5_))
CSlncR_network_GW16to23.5_ <- list(df_GW16_, df_GW19.5_, df_GW20.5_, df_GW21_, df_GW23.5_)
CSlncR_network_ <- CSlncR_network_GW16to23.5_
# rm(list= "CSlncR_network_src_","CSlncR_network_GW16_","CSlncR_network_GW19.5_","CSlncR_network_GW20.5_","CSlncR_network_GW21_","CSlncR_network_GW23.5_","CSlncR_network_GW16to23.5_","df_GW16_","df_GW19.5_","df_GW20.5_","df_GW21_","df_GW23.5_")
# save(CSlncR_network_,file = "CSlncR_network_GW16toGW23.5_.RData")
load("CSlncR_network_GW16toGW23.5_.RData")
...
## Experimentally validated cell-type-specific lncRNA-mRNA interactions, the ground-truth (lncRTarget variable) is from the literature of NPInter database.
    ...
    # random
    net_num <- c(length(rownames(CSlncR_network[[1]])),length(rownames(CSlncR_network[[2]])),length(rownames(CSlncR_network[[3]])),length(rownames(CSlncR_network[[4]])),length(rownames(CSlncR_network[[5]])))
...
# downstream [2] ASD
## ASD-related cell-type-specific lncRNA-mRNA interactions. the list of ASD-related lncRNAs and mRNAs (ASD variable) is from SFARI tools, respectively    
...
# downstream [3] interactions  ####[1] Result analysis: The lncRNA regulation in each cell type is unique####
    ## Overlap of cell-type-specific lncRNA-mRNA interactions across cells  
    ...
    # write.csv(Overlap_network,"./tmp.csv")
...
########################################################################################################################
## downstream [8] The cell type-specific lncRNAs analysis
    ...
    # write.csv(Overlap_network_rewired_lncRcelltype,"./tmp.csv")
  ...
  CSlncR_pvalue <- function(tmp,cell_type_number) {
    res_pvalue <- matrix(NA, nrow = cell_type_number, ncol = cell_type_number)
    for (i in seq(cell_type_number)) {
      for (j in seq(cell_type_number)) {
    ...
    res_predict_pvalue <- CSlncR_pvalue(tmp= CSlncR_network_lncRcelltype,cell_type_number=cell_type_number) 
    res_ASD_pvalue <- CSlncR_pvalue(tmp= CSlncR_network_lncRcelltype_ASD,cell_type_number=cell_type_number) 

    rownames(res_predict_pvalue) <- colnames(res_predict_pvalue) <- paste("Cell_type",c(1:cell_type_number),sep=" ")
    rownames(res_ASD_pvalue) <- colnames(res_ASD_pvalue) <- paste("Cell_type",c(1:cell_type_number),sep=" ")
    test.value <- matrix(0, nrow = cell_type_number, ncol = cell_type_number)
    ...
    # write.csv(lncRcelltype_conserved_FEA[[3]][[1]]@result,"./tmp.csv")
    # write.csv(lncRcelltype_conserved_Hallmark[[1]],"./tmp.csv")
    # write.csv(lncRcelltype_conserved_Cellmarker[[1]],"./tmp.csv")
    
################################################################################################################################################    
    # downstream [9] Similarity network plot   #### [1] Result analysis: The lncRNA regulation in each cell type is unique ####   
################################################################################################################################################      
## Similarity network plot in terms of cell-type-specific lncRNA-mRNA regulatory netowork    
    ...
    corrplot(CSlncR_network_Sim, method = "pie", type = "upper",title = "A  Cell-type-specific lncRNA-mRNA interactions", mar=c(0,0,2,0),diag = FALSE, cl.lim = c(0, 1), tl.cex = 1)
...
## Similarity network plot in terms of cell-type-specific hub lncRNAs
    ...
    corrplot(CSlncR_hub_Sim, method = "pie", type = "upper", title =    "B  Cell-type-specific hub lncRNAs", diag = FALSE, mar=c(0,0,2,0),cl.lim = c(0, 1), tl.cex = 1)
    
    # save 9.5 4.5 inches-pdf, 950 450 (GW16toGW23.5)

################################################################################################################################################      
    # downstream [10] ggplot plot patchwork   #### [1] Result analysis: The lncRNA regulation in each cell type is unique #### 
################################################################################################################################################    
#### The lncRNA regulation in each development is unique ####   
## Stem plots
id_=c("GW16","GW19.5","GW20.5","GW21","GW23.5")
index_net <- data.frame(value = unlist(lapply(seq(CSlncR_network), function(i) nrow(CSlncR_network[[i]]))), id = id_)

col1 <- rep("#FF9999", cell_type_number)
p1 <- ggplot(index_net, aes(x = id, y = value)) +
  geom_point(aes(color = col1), size = 2.5) +
  geom_bar(aes(fill = col1), stat = "identity", width = 0.03) +
  xlab("Development Stages") +
  ylab("Predicted numbers") +
  geom_text(aes(label = value), size = 3, hjust = 1.15) +
...
index_validated_net <- data.frame(value = round(unlist(lapply(seq(CSlncR_network_validated), function(i) nrow(CSlncR_network_validated[[i]])/nrow(CSlncR_network[[i]])*100)),3), id = id_)

col2 <- rep("plum4", cell_type_number)
p2 <- ggplot(index_validated_net, aes(x = id, y = value)) +
  geom_point(aes(color = col2), size = 2.5) +
...
index_ASD_net <- data.frame(value = round(unlist(lapply(seq(CSlncR_network_ASD), function(i) nrow(CSlncR_network_ASD[[i]])/nrow(CSlncR_network[[i]])*100)),3), id = id_)

col3 <- rep("blue", cell_type_number)
p3 <- ggplot(index_ASD_net, aes(x = id, y = value)) +
  geom_point(aes(color = col3), size = 2.5) +
...
index_ASD_hub <- data.frame(value = round(unlist(lapply(seq(hub_lncRNAs_ASD), function(i) length(hub_lncRNAs_ASD[[i]])/length(hub_lncRNAs[[i]])*100)),3), id = id_)

col4 <- rep("green", cell_type_number)
p4 <- ggplot(index_ASD_hub, aes(x = id, y = value)) +
  geom_point(aes(color = col4), size = 2.5) +
...
############################################################################################################################################################        
# downstream [11] comparison of CSlncR-Random-TargetScan #### [3] Result analysis: CSlncR is effective in predicting cell-type-specific lncRNA targets  #### 
############################################################################################################################################################      
##### downstream [11] comparison of CSlncR-Random-TargetScan  
## Stem plots
index_validated_net <- data.frame(value = unlist(lapply(seq(CSlncR_network_validated), function(i) nrow(CSlncR_network_validated[[i]])/nrow(CSlncR_network[[i]])*100)), id = c("GW16","GW19.5","GW20.5", "GW21", "GW23.5"))
index_validated_net_ <- data.frame(value = unlist(lapply(seq(CSlncR_network_validated_), function(i) nrow(CSlncR_network_validated_[[i]])/nrow(CSlncR_network_[[i]])*100)), id = c("GW16","GW19.5","GW20.5", "GW21", "GW23.5"))
index_validated_net_random <- index_validated_net
index_validated_net_random[1] <- CSlncR_network_validated_random*100
index_validated_net_LncRNA2Target <- data.frame(value = unlist(lapply(seq(CSlncR_network_validated_LncRNA2Target), function(i) nrow(CSlncR_network_validated_LncRNA2Target[[i]])/nrow(CSlncR_network_LncRNA2Target[[i]])*100)), id = c("GW16","GW19.5","GW20.5", "GW21", "GW23.5"))
index_validated_net_NPInterPrediction <- data.frame(value = unlist(lapply(seq(CSlncR_network_validated_NPInterPrediction), function(i) nrow(CSlncR_network_validated_NPInterPrediction[[i]])/nrow(CSlncR_network_NPInterPrediction[[i]])*100)), id = c("GW16","GW19.5","GW20.5", "GW21", "GW23.5"))

data1 <- rbind(index_validated_net,index_validated_net_)
data1$Methods <- c(rep("CTSlncR",cell_type_number), rep("CTSlncR without prior knowledge",cell_type_number))
data2 <- rbind(index_validated_net,index_validated_net_random)
data2$Methods <- c(rep("CTSlncR",cell_type_number), rep("Random",cell_type_number))
data3 <- rbind(index_validated_net,index_validated_net_LncRNA2Target)
data3$Methods <- c(rep("CTSlncR",cell_type_number), rep("LncRNA2Target",cell_type_number))
data4 <- rbind(index_validated_net,index_validated_net_NPInterPrediction)
data4$Methods <- c(rep("CTSlncR",cell_type_number), rep("NPInterPrediction",cell_type_number))
...
p1 <- ggplot(data1,aes(x=id,y=value,group=Methods,color=Methods))+
  ...
  xlab("Development Stages") +
  ylab("%Validated numbers") +
  labs(subtitle = "paired t-test p-value = 8.710E-06")+
...
p2 <- ggplot(data2,aes(x=id,y=value,group=Methods,color=Methods))+
  ...
  xlab("Development Stages") +
  ylab("%Validated numbers") +
  labs(subtitle = "paired t-test p-value = 8.873E-05")+
  theme(legend.position="top",
        axis.text.x = element_text(face = "bold",size=10),
        axis.text.y = element_text(face = "bold",size=10),
        axis.title.x = element_text(face = "bold",size=12),
...
p3 <- ggplot(data3,aes(x=id,y=value,group=Methods,color=Methods))+
  ...
  xlab("Development Stages") +
  ylab("%Validated numbers") +
  labs(subtitle = "paired t-test p-value = 3.153E-03")+
  theme(legend.position="top",
      axis.text.x = element_text(face = "bold",size=10),
      axis.text.y = element_text(face = "bold",size=10),
      axis.title.x = element_text(face = "bold",size=12),
...
p4 <- ggplot(data4,aes(x=id,y=value,group=Methods,color=Methods))+
  ...
  xlab("Development Stages") +
  ylab("%Validated numbers") +
  labs(subtitle = "paired t-test p-value = 7.380E-06")+
  theme(legend.position="top",
        axis.text.x = element_text(face = "bold",size=10),
        axis.text.y = element_text(face = "bold",size=10),
        axis.title.x = element_text(face = "bold",size=12),
...
mean(data2[1][1:5,]) # 0.4275737
mean(data2[1][6:10,]) # 0.05211754
t.test(data2[1][1:5,],data2[1][6:10,],paired = TRUE) # p-value = 8.873E-05
mean(data3[1][1:5,]) # 0.4275737
mean(data3[1][6:10,]) #  0.339597
t.test(data3[1][1:5,],data3[1][6:10,],paired = TRUE) # p-value = 3.153E-03
mean(data4[1][1:5,]) # 0.4275737
mean(data4[1][6:10,]) # 0.01225273
t.test(data4[1][1:5,],data4[1][6:10,],paired = TRUE) # p-value = 7.380E-06
